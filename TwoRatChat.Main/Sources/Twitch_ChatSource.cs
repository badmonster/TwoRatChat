...
namespace TwoRatChat.Main.Sources
{
    ...
    public class Twitch_ChatSource : TwoRatChat.Model.ChatSource
    {
        ...
        //там где IrcClient connected ещё коммент readonly Timer _pingTimer;
        //  TwitchFollowersController _controller;
        string _id;

        // old readonly string _apiKey = "Здесь ТВИЧЁВСКИЙ API KEY";        
        private readonly string _apiKey = Properties.Settings.Default.twitchAPIKey;
        private string twitchtoken1 = Properties.Settings.Default.Twitchaccesstoken;
        ...
        */
            ...
            : base(dispatcher)
        {
            ...
            // 3 части в разных местах _pingTimer = new Timer(5000);
        ...
        private void Reconnect()
        {
            ...
            //это было раньше закомменчено IrcClient. = "TWITCHCLIENT 3";
            //_ircClient.PongReceived += IrcClient_PongReceived;
            //_ircClient.PingReceived += IrcClient_PingReceived;
            _ircClient.Connected += IrcClient_Connected;
            _ircClient.ProtocolError += IrcClient_ProtocolError;
 ...
 */
        ...
        {
            HashSet<int> x;
            if (!_emotesets.TryGetValue(nick, out x))
        ...
        void IrcClient_RawMessageReceived(object sender, IrcRawMessageEventArgs e)
        {
            ...
            try
            {
                ...
                switch (command)
                {
                    /* case "001":
                         this.Status = true;
                         this.Tooltip = string.Format( "twitch channel {0} not found!", _StreamerNick );
                         Destroy();
                         break;*/

                    case "PART":
                    case "JOIN":
                        //Header = "http://twitch.tv, " + _StreamerNick + dots;
                        this.Status = true;
                        this.Tooltip = string.Format("twitch: {0}", _streamerNick);
                        //this.Header = "Joined";
                        this.Header = "Joined";
                    ...
                    case "PRIVMSG":
                        //Header = "http://twitch.tv, " + _StreamerNick;
                        if (username != "Jtv")
                        {
            ...
            if( _allowUpdateCount ) {
                ...
                if( !string.IsNullOrEmpty( _streamerNick ) ) {
                    if( string.IsNullOrEmpty( userID ) ) {
                        this.Header = "Find ID...";
                        WebClient wc = new WebClient();
                        wc.Headers.Add("Authorization: Bearer " + twitchtoken1);
                            ...
                            if( b.Error == null ) {
                                try {
                                    ...
                                    if( response._total == 1 ) {
                                        ...
                                        this.Header = "Loading..";
                                        App.Log( ' ', "Twitch Find ID: {0} is {1}", _streamerNick, userID );
                                    } else {
                        ...
                        wc.DownloadStringAsync( new System.Uri("https://api.twitch.tv/helix/streams?user_login=" + _streamerNick ) );
                    } else {
                        WebClient wc = new WebClient();
                        //wc.Headers.Add( "Accept: application/vnd.twitchtv.v5+json" );
                        wc.Headers.Add("Authorization: Bearer " + twitchtoken1);
                        ...
                        wc.DownloadStringAsync(new System.Uri("https://api.twitch.tv/helix/users/follows?to_id=" + userID));
                    }
                }
            }
        }*/
        bool _inCountUpdate = false;
        bool _inFollowUpdate = false;
        DateTime _lastFollowerUpdate = DateTime.Now.ToUniversalTime();


               
        int _userID = Convert.ToInt32(Settings.Default.TwitchChannelID);

        int _followers = 0;
        
        void UpdateHeaderCount()
        {
            Header = $"{ViewersCount}/{_followers}";
        }
        ...
        void PopulateViewerCount(bool repopulateFollowers)
        {
            
            // Get view count and userID from nickname.
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                //wc.UseDefaultCredentials.ToString();
                //WebHeaderCollection.IsRestricted("Authorization", "Bearer " + twitchtoken1);
                wc.Headers.Add("Authorization: Bearer " + twitchtoken1);
                //wc.Headers.Add("Accept: application/vnd.twitchtv.v5+json");
                wc.Headers.Add("Client-ID", _apiKey);
                wc.DownloadStringCompleted += (a, b) =>
                {
                    ...
                    if (b.Error == null)
                    {
                        ...
                        {
                            ViewersCount = (int)response.data[0].viewer_count;

                            if (response.data[0].user_id != null)
                            {
                                _userID = (int)response.data[0].user_id;

                                if (repopulateFollowers)
                                    PopulateFollowersCount(_userID);
                            }
        ...
        void PopulateFollowersCount(int _userID)
        {
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                wc.DownloadStringAsync(new System.Uri("https://api.twitch.tv/helix/users/follows?to_id=" + _userID));
            }
        }

        bool _repopulateFollowers = false;
